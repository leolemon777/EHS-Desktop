(() => {
  ...
  const moduleConfigs = {
    tools: {
      ...
      stats(rows) {
        ...
        return [["工具总数", rows.length, "wrench", "台账中登记的工具"], ["在用工具", active, "circle-check", "尚未填写报废时间"], ["已报废", scrapped, "archive", "已填写报废时间"]];
      }
    },
    energy: {
      title: "能源数据",
      ...
      stats(rows) { return [["计量点", rows.length, "gauge", "本月已纳入统计"], ["已完成抄表", count(rows, "status", "已抄表"), "circle-check", "数据已经确认"], ["需要复核", count(rows, "status", "需复核"), "search", "存在异常波动"]]; }
    },
    certificates: {
      title: "人员证件",
      ...
      stats(rows) { return [["证件总数", rows.length, "credit-card", "四类人员证件"], ["即将到期", count(rows, "status", "即将到期"), "clock", "建议提前办理复审"], ["已经过期", count(rows, "status", "已过期"), "octagon-alert", "需要立即处理"]]; }
    },
    waste: {
      title: "危废存量",
      ...
      stats(rows) { return [["危废种类", rows.length, "recycle", "当前在库类别"], ["库存总量", `${sum(rows, "quantity").toFixed(2)} 吨`, "scale", "按当前台账汇总"], ["库存预警", count(rows, "status", "接近上限"), "triangle-alert", "库容超过预警阈值"]]; }
    },
    chemicals: {
      title: "化学品管理",
      ...
      stats(rows) { return [["化学品种类", rows.length, "flask-conical", "当前登记品种"], ["SDS 覆盖率", `${rows.length ? Math.round(rows.filter(row => row.sds).length / rows.length * 100) : 0}%`, "file-text", "已建立 SDS 档案"], ["需要关注", rows.filter(r => r.status !== "在库").length, "triangle-alert", "低库存或待复核"]]; }
    },
    inspections: {
      title: "工具巡检",
      ...
      stats(rows) {
        ...
        return [["巡检次数", rows.length, "clipboard-list", "已建立的检查记录"], ["已完成", completed, "circle-check", "已填写检查责任人和日期"], ["待补录", pending, "triangle-alert", "仍需补充表内信息"]];
      }
    }
  };
  ...
  function refreshIcons(root) {
    ...
    if (root && root !== document) lucide.createIcons({ root });
  ...
  function renderDateField(name, label, type, value = "", { className = "", required = true } = {}) {
    ...
    return `
      <label class="field field--date ${escapeHtml(className)}">
        ...
        <span class="date-control" data-date-control data-date-type="${type}" data-date-value="${escapeHtml(normalized)}" data-date-view="${escapeHtml(view)}">
          <span class="date-control__input-wrap">
            <input class="input date-control__display" name="${escapeHtml(name)}" type="text" value="${escapeHtml(formatDateValue(normalized, type))}" placeholder="${placeholder}" readonly${required ? " required" : ""} aria-haspopup="dialog" aria-expanded="false" data-date-display autocomplete="off" inputmode="none" />
            <button class="date-control__button" type="button" data-date-trigger aria-label="打开${escapeHtml(label)}选择器"><i data-lucide="calendar"></i></button>
          </span>
  ...
  function renderDatePicker(control) {
    ...
    picker.innerHTML = `<div class="date-picker__header"><button type="button" class="date-picker__nav" data-date-nav="-1" aria-label="上一个${navLabel}"><i data-lucide="chevron-left"></i></button><strong>${headerLabel}</strong><button type="button" class="date-picker__nav" data-date-nav="1" aria-label="下一个${navLabel}"><i data-lucide="chevron-right"></i></button></div>${content}`;
    refreshIcons(picker);
    ...
    return `
      <header class="page-heading">
        ...
        <div class="page-heading__actions">
          ${supportsWordExport ? '<button class="btn btn-secondary" type="button" data-action="export-word"><i data-lucide="file-text" class="icon"></i> 导出 Word</button>' : ""}
          <button class="btn btn-secondary" type="button" data-action="export-csv"><i data-lucide="download" class="icon"></i> 导出 CSV</button>
          ${withAdd ? '<button class="btn btn-primary" type="button" data-action="add"><i data-lucide="plus" class="icon"></i> 新增记录</button>' : ""}
        </div>
      </header>`;
  }
  ...
  function renderOverview() {
    ...
    const modules = [
      ["tools", "wrench", "手持电动工具", "317-03b 原表台账"],
      ["inspections", "clipboard-list", "工具巡检", "标准检查项与周期记录"],
      ["energy", "zap", "能源数据", "水、电、气读数"],
      ["certificates", "id-card", "人员证件", "证件有效期管理"],
      ["waste", "recycle", "危废存量", "库存与处置状态"],
      ["chemicals", "flask-conical", "化学品管理", "SDS 与库存信息"]
    ];
    const moduleCards = modules.map(([route, icon, title, description]) => {
      ...
      return `<button class="dashboard-module-card ${count ? "is-started" : ""}" type="button" data-route-go="${route}">
        <span class="dashboard-module-card__icon"><i data-lucide="${icon}"></i></span>
        ...
        <i data-lucide="arrow-up-right" class="dashboard-module-card__arrow"></i>
      </button>`;
    }).join("");
    const alertRows = [
      ["certificates", "id-card", "人员证件", "到期及即将到期", certAlerts],
      ["tools", "wrench", "手持电动工具", "编号、责任人等字段待补录", toolAlerts],
      ["waste", "recycle", "危废存量", "接近库容上限", wasteAlerts]
    ].map(([route, icon, title, description, value]) => `<button class="dashboard-alert" type="button" data-route-go="${route}">
      <span class="dashboard-alert__icon"><i data-lucide="${icon}"></i></span>
      <span><strong>${title}</strong><small>${description}</small></span>
      <b>${value}</b><i data-lucide="chevron-right"></i>
    </button>`).join("");
    pageContent.innerHTML = `
      ${pageHeading("管理总览", "", "以仪表盘方式查看净水工厂的 EHS 数据准备度、风险提醒和业务入口。", false)}
      <section class="dashboard-kpi-grid" aria-label="EHS 核心指标">
        <article class="dashboard-kpi dashboard-kpi--accent"><span class="dashboard-kpi__icon"><i data-lucide="files"></i></span><span><small>业务台账</small><strong>${totalRecords}</strong><span class="dashboard-kpi__note">当前已录入的真实记录</span></span></article>
        <article class="dashboard-kpi dashboard-kpi--warning"><span class="dashboard-kpi__icon"><i data-lucide="circle-alert"></i></span><span><small>待处理事项</small><strong>${alertTotal}</strong><span class="dashboard-kpi__note">${alertTotal ? "需要尽快跟进" : "当前没有待处理提醒"}</span></span></article>
        <article class="dashboard-kpi"><span class="dashboard-kpi__icon"><i data-lucide="layout-grid"></i></span><span><small>已启用模块</small><strong>${activeModules}<b>/${moduleCount}</b></strong><span class="dashboard-kpi__note">已有真实数据的模块</span></span></article>
        <article class="dashboard-kpi"><span class="dashboard-kpi__icon"><i data-lucide="chart-pie"></i></span><span><small>数据覆盖率</small><strong>${coverage}%</strong><span class="dashboard-kpi__note">按模块录入进度计算</span></span></article>
      </section>
      <section class="dashboard-grid dashboard-grid--top">
        <article class="dashboard-panel dashboard-readiness">
          <header class="dashboard-panel__head"><div><h2>合规数据准备度</h2></div><span class="dashboard-panel__meta">${activeModules}/${moduleCount} 个模块已开始</span></header>
          <div class="dashboard-readiness__body">
            ...
            <div class="dashboard-readiness__copy"><strong>${coverage ? "管理数据正在建立" : "从第一条真实记录开始"}</strong><p>${coverage ? "已开始录入的模块会在这里持续汇总。" : "当前尚未录入业务数据，录入后指标、提醒和趋势会自动更新。"}</p><button class="btn btn-primary" type="button" data-action="add"><i data-lucide="plus"></i> 开始录入</button></div>
          </div>
        </article>
        <article class="dashboard-panel dashboard-alerts">
          <header class="dashboard-panel__head"><div><h2>风险与提醒</h2></div><span class="dashboard-count">${alertTotal}</span></header>
          <div class="dashboard-alert-list">${alertRows}</div>
        </article>
      </section>
      <section class="dashboard-section-head"><div><h2>业务模块</h2></div><span>${activeModules} / ${moduleCount} 已开始</span></section>
      ...
      <section class="dashboard-grid dashboard-grid--lower">
        <article class="dashboard-panel dashboard-trend"><header class="dashboard-panel__head"><div><h2>能源使用趋势</h2><p>录入能源数据后自动生成</p></div><button class="panel__link" type="button" data-route-go="energy">前往能源数据 <i data-lucide="arrow-up-right"></i></button></header><div class="dashboard-chart-empty"><span class="dashboard-chart-empty__icon"><i data-lucide="chart-line"></i></span><strong>等待真实能源数据</strong><p>添加抄表记录后，这里会显示水、电、气的变化趋势。</p></div></article>
        <article class="dashboard-panel dashboard-quick"><header class="dashboard-panel__head"><div><h2>快速入口</h2></div></header><div class="dashboard-quick-list"><button type="button" data-route-go="certificates"><span class="dashboard-quick-list__icon"><i data-lucide="id-card"></i></span><span><strong>人员证件</strong><small>管理健康证、叉车证等</small></span><i data-lucide="arrow-right"></i></button><button type="button" data-route-go="tools"><span class="dashboard-quick-list__icon"><i data-lucide="wrench"></i></span><span><strong>手持电动工具</strong><small>维护 317-03b 原表字段</small></span><i data-lucide="arrow-right"></i></button><button type="button" data-route-go="chemicals"><span class="dashboard-quick-list__icon"><i data-lucide="flask-conical"></i></span><span><strong>化学品管理</strong><small>维护 SDS 与库存信息</small></span><i data-lucide="arrow-right"></i></button></div></article>
      </section>`;
    refreshIcons(pageContent);
  }

  function attentionItem(icon, title, text, value, route) {
    return `<button class="attention-item" type="button" data-route-go="${route}" style="border:0;text-align:left;cursor:pointer"><span class="attention-item__icon"><i data-lucide="${icon}"></i></span><span><strong>${title}</strong><small>${text}</small></span><b>${value}</b></button>`;
  }

  function metricCard(icon, value, label, trend, warning = false) {
    const neutral = trend === "暂无记录";
    return `<article class="metric-card"><div class="metric-card__top"><span class="metric-card__icon"><i data-lucide="${icon}"></i></span><span class="metric-card__trend ${neutral ? "is-neutral" : warning ? "is-warning" : ""}"><i data-lucide="${neutral ? "minus" : warning ? "triangle-alert" : "trending-down"}"></i>${escapeHtml(trend)}</span></div><strong>${escapeHtml(value)}</strong><p>${escapeHtml(label)}</p></article>`;
  }
  ...
  function renderModule(route) {
    ...
    pageContent.innerHTML = `
      ${pageHeading(config.title, "", config.description)}
      <section class="module-stats">
        ${config.stats(data[route]).map(stat => `<article class="stat-tile"><div class="stat-tile__head"><span>${escapeHtml(stat[0])}</span><i data-lucide="${stat[2]}"></i></div><strong>${escapeHtml(stat[1])}</strong><small>${escapeHtml(stat[3])}</small></article>`).join("")}
      </section>
      <section class="panel table-panel">
        <div class="table-toolbar">
          <div class="filter-group">
            ...
            ${(currentStatus !== "all" || currentQuery) ? '<button class="btn btn-ghost" type="button" data-action="reset-filters"><i data-lucide="rotate-ccw"></i> 重置</button>' : '<span class="chip"><i data-lucide="funnel"></i> 实时筛选</span>'}
          </div>
  ...
  function renderFormModule(route) {
    ...
    if (form.layout === "inspection") {
      ...
      if (selectedRecord) {
        ...
        pageContent.innerHTML = `
          <header class="page-heading inspection-heading">
            <div class="page-heading__copy">
              <button class="inspection-back" type="button" data-inspection-back><i data-lucide="arrow-left"></i> 返回巡检记录</button>
              <span class="inspection-page-tag"><i data-lucide="tag"></i><span data-inspection-label-display>${escapeHtml(selectedRecord.uiLabel || "未设置标签")}</span></span>
            ...
            <div class="page-heading__actions">
              <button class="btn btn-secondary" type="button" data-inspection-nav="previous" ${previous ? "" : "disabled"}><i data-lucide="chevron-left"></i> 上一张</button>
              <button class="btn btn-secondary" type="button" data-inspection-nav="next" ${next ? "" : "disabled"}>下一张 <i data-lucide="chevron-right"></i></button>
              <button class="btn btn-primary" type="button" data-action="form-add"><i data-lucide="plus"></i> 新增一张</button>
            </div>
          </header>
          <section class="inspection-editor">
            <div class="inspection-editor__bar">
              <div class="inspection-editor__identity">
                <span class="inspection-editor__saved"><i data-lucide="badge-check"></i><span data-tool-sync-state>${selectedRecord.ledgerToolId && data.tools.some(tool => tool.id === selectedRecord.ledgerToolId) ? "已同步工具台账" : "填写编号和名称后同步台账"}</span></span>
              ...
              <div>
                <button class="inspection-editor__action" type="button" data-form-print="${escapeHtml(selectedRecord.id)}"><i data-lucide="printer"></i> 打印本表</button>
                <button class="inspection-editor__action" type="button" data-action="export-word"><i data-lucide="file-text"></i> 批量导出 Word</button>
                <button class="inspection-editor__action inspection-editor__action--danger" type="button" data-form-delete="${escapeHtml(selectedRecord.id)}"><i data-lucide="trash-2"></i> 删除本表</button>
              </div>
            </div>
      ...
      pageContent.innerHTML = `
        <header class="page-heading">
          ...
          <div class="page-heading__actions">
            <button class="btn btn-primary" type="button" data-action="form-add"><i data-lucide="plus"></i> ${addLabel}</button>
            <button class="btn btn-secondary" type="button" data-action="export-word"><i data-lucide="file-text"></i> 批量导出 Word</button>
            <button class="btn btn-secondary" type="button" data-action="export-csv"><i data-lucide="download"></i> 导出 CSV</button>
          </div>
        </header>
        <section class="inspection-directory">
          <div class="inspection-directory__toolbar">
            <label class="inspection-search">
              ...
              ${inspectionListQuery ? '<button type="button" data-inspection-search-clear aria-label="清空搜索"><i data-lucide="x"></i></button>' : ""}
            </label>
          ...
          ${visibleRows.length ? `
            ...
            <div class="inspection-empty">
              <span><i data-lucide="${rows.length ? "search" : "clipboard-list"}"></i></span>
              <h2>${rows.length ? "没有匹配的巡检记录" : "还没有巡检记录"}</h2>
              <p>${rows.length ? "换一个工具编号、名称、部门或检查人试试。" : "新建后只打开当前这一张表，填写内容会自动保存在本机。"}</p>
              ${rows.length ? '<button class="btn btn-secondary" type="button" data-inspection-search-clear>清空搜索</button>' : '<button class="btn btn-primary" type="button" data-action="form-add"><i data-lucide="plus"></i> 新建第一张</button>'}
            </div>`}
        </section>`;
      return;
    }
    ...
    pageContent.innerHTML = `
      <header class="page-heading">
        ...
        <div class="page-heading__actions">
          <button class="btn btn-secondary" type="button" data-action="export-word"><i data-lucide="file-text"></i> 导出 Word</button>
          <button class="btn btn-secondary" type="button" data-action="export-csv"><i data-lucide="download"></i> 导出 CSV</button>
        </div>
      </header>
      <section class="module-stats">
        ${config.stats(rows).map(stat => `<article class="stat-tile"><div class="stat-tile__head"><span>${escapeHtml(stat[0])}</span><i data-lucide="${stat[2]}"></i></div><strong>${escapeHtml(stat[1])}</strong><small>${escapeHtml(stat[3])}</small></article>`).join("")}
      </section>
      <div class="tabs" role="tablist">
        <button class="tab ${isFormMode ? "is-active" : ""}" type="button" role="tab" data-form-tab="form"><i data-lucide="pen-line"></i> ${formTabLabel}</button>
        <button class="tab ${!isFormMode ? "is-active" : ""}" type="button" role="tab" data-form-tab="list"><i data-lucide="list"></i> 全部列表</button>
      </div>
      ${isFormMode ? `
        <section class="panel form-sheet-panel">
          ...
          <div class="form-sheet__body">
            <table class="form-sheet__table">
              ...
              <tbody>
                ${expandedContent || (rows.length ? "" : `<tr class="form-sheet__empty"><td colspan="${form.sheetColumns.length + 1}"><div class="empty-state"><span class="empty-state__icon"><i data-lucide="pen-line"></i></span><h3>开始填写第一行</h3><p>点击下方「新增一行」，在表格里直接录入工具信息。</p></div></td></tr>`)}
              </tbody>
            </table>
      ...
      ` : `
        <section class="panel table-panel">
          <div class="table-toolbar">
            <span class="chip"><i data-lucide="funnel"></i> ${currentQuery ? "搜索结果" : "全部记录"}</span>
  ...
  function renderInspectionListItem(record, no) {
    ...
    return `
      <article class="inspection-record" data-inspection-open="${escapeHtml(record.id)}">
        ...
        <span class="inspection-record__icon"><i data-lucide="clipboard-list"></i></span>
        <span class="inspection-record__main">
          <span class="inspection-record__title">
            <strong>${escapeHtml(record.toolName || "未命名检查记录")}</strong>
            <span class="inspection-record__tag ${record.uiLabel ? "" : "is-empty"}"><i data-lucide="tag"></i>${escapeHtml(record.uiLabel || "未设置标签")}</span>
            <span class="inspection-record__status ${isReady ? "is-ready" : ""}">${isReady ? "基本信息完整" : `待补充 ${keyFields.length - filled} 项`}</span>
          </span>
          <span class="inspection-record__meta">
            <span><i data-lucide="hash"></i>${escapeHtml(record.toolRef || "未填写工具编号")}</span>
            <span><i data-lucide="building-2"></i>${escapeHtml(record.dept || "未填写部门")}</span>
            <span><i data-lucide="user"></i>${escapeHtml(record.inspector || "未填写检查人")}</span>
          </span>
        </span>
        ...
        <span class="inspection-record__actions">
          <button class="inspection-record__delete" type="button" data-form-delete="${escapeHtml(record.id)}" aria-label="删除${escapeHtml(record.toolName || "该巡检记录")}"><i data-lucide="trash-2"></i></button>
          <button class="inspection-record__open" type="button" data-inspection-open="${escapeHtml(record.id)}">打开填写 <i data-lucide="arrow-right"></i></button>
        </span>
      </article>`;
  }
  ...
  function renderFormCard(route, record, no) {
    ...
    return `
      <article class="form-card" data-form-card="${escapeHtml(record.id)}">
        ...
        <span class="form-card__icon"><i data-lucide="${config.icon}"></i></span>
        ...
        <span class="form-card__actions">
          <button class="row-action" type="button" data-form-print="${escapeHtml(record.id)}" aria-label="打印"><i data-lucide="printer"></i></button>
          <button class="row-action" type="button" data-form-expand="${escapeHtml(record.id)}" aria-label="展开编辑"><i data-lucide="pencil"></i></button>
          <button class="row-action" type="button" data-form-delete="${escapeHtml(record.id)}" aria-label="删除"><i data-lucide="trash-2"></i></button>
        </span>
      </article>`;
  }
    ...
    return `
      <tr class="form-row form-row--editing" data-form-row="${escapeHtml(record.id)}">
        ...
        <td class="form-sheet__cell form-sheet__cell--action">
          <button class="row-action" type="button" data-form-print="${escapeHtml(record.id)}" aria-label="打印"><i data-lucide="printer"></i></button>
          <button class="row-action" type="button" data-form-collapse="${escapeHtml(record.id)}" aria-label="完成折叠"><i data-lucide="check"></i></button>
        </td>
      </tr>${extraRow}`;
  }
      ...
      <article class="word-sheet" data-form-row="${escapeHtml(rid)}">
        ...
        <div class="word-sheet__sign">
          ...
          <span class="word-sheet__ops">
            <button class="word-op" type="button" data-form-print="${escapeHtml(rid)}" aria-label="打印本表"><i data-lucide="printer"></i> 打印</button>
            <button class="word-op word-op--danger" type="button" data-form-delete="${escapeHtml(rid)}" aria-label="删除本表"><i data-lucide="trash-2"></i></button>
          </span>
        </div>
      </article>`;
  }
  ...
  function renderTable(route, rows) {
    ...
    if (!rows.length) {
      ...
      return `<div class="empty-state"><span class="empty-state__icon"><i data-lucide="${filtered ? "search" : config.icon}"></i></span><h3>${filtered ? "没有找到匹配记录" : "尚无业务数据"}</h3><p>${filtered ? "尝试更改搜索关键词或状态筛选条件。" : "点击右上角“新增记录”，录入第一条真实数据。"}</p></div>`;
    }
    return `
      <div class="data-table-wrap">
        <table class="data-table">
          ...
          <tbody>
            ${rows.map(row => `<tr data-record-id="${escapeHtml(row.id)}">${config.columns.map((column, index) => `<td data-label="${escapeHtml(column[1])}">${renderCell(config, row, column[0], index)}</td>`).join("")}<td data-label="操作"><div class="row-actions"><button class="row-action" type="button" data-row-view="${escapeHtml(row.id)}" aria-label="查看详情"><i data-lucide="eye"></i></button><button class="row-action" type="button" data-row-more="${escapeHtml(row.id)}" aria-label="更多操作"><i data-lucide="more-horizontal"></i></button></div></td></tr>`).join("")}
          </tbody>
        </table>
      </div>
      <footer class="table-footer"><span>显示 1–${rows.length} 条，共 ${rows.length} 条</span><div class="pagination"><button class="page-btn" type="button"><i data-lucide="chevron-left"></i></button><button class="page-btn is-active" type="button">1</button><button class="page-btn" type="button"><i data-lucide="chevron-right"></i></button></div></footer>`;
  }

  function renderCell(config, row, key, index) {
    const value = row[key];
    if (index === 0) return `<div class="primary-cell"><span class="primary-cell__icon"><i data-lucide="${config.icon}"></i></span><span><strong>${escapeHtml(value)}</strong><small>${escapeHtml(row[config.primary[1]])}</small></span></div>`;
    ...
    if (key === "compare") return `<span style="color:${String(value).startsWith("+") ? "var(--color-citrus)" : "var(--color-meadow)"};font-weight:700">${escapeHtml(value)}</span>`;
  ...
  function renderField([name, label, type, detail], value = "") {
    if (type === "heading") return `<div class="field is-wide inspection-group"><span class="inspection-group__title">${escapeHtml(label)}</span></div>`;
    if (type === "person") return renderPersonField(name, label, value, detail);
    if (type === "select") return `<div class="field field--select"><span class="field-label">${escapeHtml(label)}</span>${renderSelectControl(name, detail, value, { label, className: "select-control--field" })}</div>`;
    if (type === "date" || type === "month") return renderDateField(name, label, type, value);
    return `<label class="field"><span class="field-label">${escapeHtml(label)}</span><input class="input" name="${escapeHtml(name)}" type="${escapeHtml(type)}" value="${escapeHtml(value)}" ${detail ? `placeholder="${escapeHtml(detail)}"` : ""} required /></label>`;
  }

  function renderPersonField(name, label, value = "", placeholder = "请输入姓名") {
    const suggestions = personNameSuggestions(value);
    return `<div class="field field--person"><span class="field-label">${escapeHtml(label)}</span>
      <span class="person-control" data-person-control>
        <span class="person-input-wrap">
          <input class="input person-input" name="${escapeHtml(name)}" type="text" value="${escapeHtml(value)}" placeholder="${escapeHtml(placeholder)}" autocomplete="off" required data-person-input aria-haspopup="listbox" aria-expanded="false" />
          <button class="person-trigger" type="button" data-person-trigger aria-label="选择已保存姓名" aria-expanded="false"><i data-lucide="users" aria-hidden="true"></i></button>
        </span>
        <span class="person-menu" role="listbox" data-person-menu hidden>${renderPersonMenu(suggestions, value)}</span>
      </span>
    </div>`;
  }
  ...
  function renderPersonMenu(suggestions, query = "") {
    ...
    const content = suggestions.length
      ? suggestions.map(name => `<button class="person-option" type="button" role="option" data-person-option data-person-value="${escapeHtml(name)}"><span class="person-option__avatar">${escapeHtml(name.slice(0, 1))}</span><span>${escapeHtml(name)}</span><i data-lucide="arrow-up-left" aria-hidden="true"></i></button>`).join("")
      : `<div class="person-empty"><i data-lucide="user-plus" aria-hidden="true"></i><span>${query ? "没有匹配的已保存姓名" : "保存第一条人员证件后，这里会出现姓名"}</span></div>`;
    return `<div class="person-menu__head"><span>${title}</span><small>${suggestions.length ? `${suggestions.length} 个` : "本机保存"}</small></div>${content}`;
  }
  ...
  function renderSelectControl(name, options, value = "", { label = "", className = "", required = true } = {}) {
    ...
    return `<span class="select-control ${escapeHtml(className)}" data-select-control>
      <input class="select__native" type="hidden"${nameAttribute} value="${escapeHtml(selected.value)}" data-required="${required}" />
      <button class="select-trigger" type="button" data-select-trigger aria-haspopup="listbox" aria-expanded="false"${label ? ` aria-label="${escapeHtml(label)}"` : ""}>
        <span data-select-value>${escapeHtml(selected.label)}</span><i data-lucide="chevron-down" aria-hidden="true"></i>
      </button>
      <span class="select-menu" role="listbox" hidden>
        ${normalizedOptions.map(option => `<button class="select-option${option.value === selected.value ? " is-selected" : ""}" type="button" role="option" data-select-option data-select-value-option="${escapeHtml(option.value)}" aria-selected="${option.value === selected.value}"><span>${escapeHtml(option.label)}</span><i data-lucide="check" aria-hidden="true"></i></button>`).join("")}
      </span>
    </span>`;
  }
  ...
  function selectCustomOption(control, option) {
    const nativeSelect = control.querySelector(".select__native");
  ...
  function openDetail(route, recordId) {
    ...
    document.querySelector("#detailBody").innerHTML = `
      <div class="detail-hero"><span class="detail-hero__icon"><i data-lucide="${config.icon}"></i></span><span><strong>${escapeHtml(record[config.primary[0]])}</strong><small>${escapeHtml(record[config.primary[1]] || record.id)}</small></span></div>
      <div class="detail-grid">${detailRows}</div>
      <div class="detail-note"><i data-lucide="info"></i><span>编辑后会立即保存到当前浏览器，删除操作可在提示出现期间撤销。</span></div>`;
    refreshIcons(detailDrawer);
    ...
    if (!query) {
      content = Object.entries(moduleConfigs).map(([route, config]) => `
        <button class="search-result" type="button" data-search-route="${route}">
          <span class="search-result__icon"><i data-lucide="${config.icon}"></i></span>
      ...
      if (!content) content = `<div class="search-empty"><i data-lucide="search"></i>没有找到与“${escapeHtml(searchInput.value.trim())}”匹配的记录</div>`;
    }
    searchPanel.innerHTML = `<div class="search-panel__head"><span>${query ? "全局搜索结果" : "快速前往"}</span><kbd>ESC 关闭</kbd></div>${content}`;
    refreshIcons(searchPanel);
  ...
  pageContent.addEventListener("change", event => {
    ...
    const formSelect = event.target.closest(".form-cell-select [data-select-control] .select__native, .form-extra-field [data-select-control] .select__native");
